...
// const int WINDOW_SIZE = 1024;      // Kích thước cửa sổ NFFT
// #define HOP_SIZE 256              // Độ chồng của các cửa sổ
...
// float* audioBuffer = (float*)ps_calloc(TOTAL_SAMPLES, sizeof(float));
...
// // Tensor Arena
// constexpr int kTensorArenaSize = 3 * 1024 * 1024; // 128 KB
...
//                 audioBuffer[bufferWritePos++] = normalized;
...
//             // Nếu đã có đủ BUFFER_SAMPLES mẫu → báo task xử lý
...
//                 // Serial.println(processingBuffer[i],6);
//             }
...
//             for (int frame = 0; frame < NUM_FRAMES; frame++) {
//                 for (int bin = 0; bin < fftBins -1 ; bin++) {
//                     // Serial.println(stftFrames[frame].real[bin], 6);
...
//             }
//   //             for (int i = 0; i < 100; i++) {
//   //     Serial.println(input->data.f[i], 6);
//   // }
...
// //             Serial.print("Shape của output tensor: ");
// //             for (int i = 0; i < output->dims->size; i++) { 
// //                 Serial.print(output->dims->data[i]);
// //                 if (i < output->dims->size - 1) Serial.print(" x ");
// //             }
// //             Serial.println();
// //             int outputBytes = output->bytes;
// // int outputElements = outputBytes / sizeof(float);
// //             Serial.println("Output data:");
// // for (int i = 0; i < outputElements; i++) {
// //     Serial.println(outputData[i], 6);  // In 6 chữ số sau dấu phẩy
// // }
//               inputIdx = 0;
//               for (int frame = 0; frame < NUM_FRAMES; frame++) {
//                   for (int bin = 0; bin < fftBins; bin++) {
//                       float realVal = outputData[inputIdx++];
//                       float imagVal = outputData[inputIdx++];

//                       outputSTFTFrames[frame].real[bin] = realVal;
//                       outputSTFTFrames[frame].imag[bin] = imagVal;
//                   }

//                   // Mirror từ bin = 1 đến fftBins - 2
...
//     xTaskCreatePinnedToCore(
//         processingTask, "ProcessingTask", 32768, NULL, 2, NULL, 1);
